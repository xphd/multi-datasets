...
module.exports = function(
  ...
  if (fs.existsSync("/input/")) {
    datasetPath = "/input/";
    console.log(
      "tinyconf: running in container, searching for datasets in",
      datasetPath
    );
  } else {
    console.log(
      "tinyconf: running locally, search for datasets in",
      datasetPath
    );
  }

  let conf = getConf(datasetPath, selected_dataset);
  ...
  function getConf(datasetPath, selected_dataset) {
    var dataset_ident = selected_dataset;
    ...
    if (dataset_ident === null) {
      ...
      console.log(
        "\x1b[34m%s\x1b[0m",
        "dataset folder mounted directly at /input/"
      );
      files = fs
      ...
      if (files.length === 1) {
        ...
        console.log(
          "\x1b[34m%s\x1b[0m",
          "no basename found; assuming config file"
        );
      ...
      if (dataset_ident.startsWith("dev:")) {
        ...
        console.log(
          "tinyconf: switching to dev mode with dataset ident:",
          dataset_ident
        );
      }
      files = fs
        .readdirSync(datasetPath)
        .filter(filename =>
          filename.toLowerCase().includes(dataset_ident.toLowerCase())
        );
      if (files.length === 0) {
        console.log(
          "\x1b[34m%s\x1b[0m",
          "no dataset found, using default config"
        );
        return defaultConf;
      } else {
        if (files.length > 1) {
        ...
        return dev_mode
          ? createConf(datasetPath, files[0])
          : createConfEval(datasetPath, files[0], false);
      }
    }
    ...
    function createConfEval(datasetPath, basename, directMount) {
      ...
      if (basename.length === 0) {
        ...
        if (existingConfig) {
          ...
          let problem_schema = existingConfig.problem_schema
            ? existingConfig.problem_schema
            : datasetPath + prob + "/problemDoc.json";
          let ret = {
